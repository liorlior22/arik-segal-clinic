    ...
    <main>
      ...
      <section className="hero" id="top">
        <div className="heroCopy">
          ...
          <p className="lead">ייעוץ פרטי מקצועי ומעמיק, עם הסתכלות רחבה על התמונה הרפואית וליווי אישי בגובה העיניים.</p>
          ...
          <div className="heroMeta">
            ...
            <div><span>זימון תורים</span><strong>דרך אסותא</strong></div>
          </div>
        </div>

        <div className="heroVisual">
          <div className="heroImageFrame">
            <Image className="heroImage" src="/arik-procedure.png" alt="ד״ר אריק סגל" width={760} height={800} priority sizes="(max-width: 900px) 100vw, 48vw" />
          </div>
      ...
      <section className="section specialtiesSection" id="specialties">
        <div className="sectionHead splitHead">
          ...
          <p>הבנת התמונה המלאה, מעבר על הבדיקות הקיימות ובניית המשך בירור וטיפול מותאם אישית.</p>
        </div>

        <div className="cards">
          {specialties.map((item, index) => (
            <article className="card" key={item.title}>
              <div className="cardTop"><span className="cardNum">0{index + 1}</span><span className="cardArrow">↗</span></div>
              <h3>{item.title}</h3>
              <p>{item.text}</p>
      ...
      <section className="about section" id="about">
        <div className="portraitColumn">
          <div className="portraitWrap">
            <Image className="portrait" src="/arik-portrait.png" alt="ד״ר אריק סגל" width={440} height={440} sizes="(max-width: 900px) 70vw, 32vw" />
          </div>
        ...
        <div className="aboutCopy">
          ...
          <p className="aboutLead">ד״ר אריק סגל סיים את לימודי הרפואה בטכניון בשנת 2002. התמחה ברפואה פנימית במרכז הרפואי רמב״ם ובהמשך בגסטרואנטרולוגיה ומחלות כבד במרכז הרפואי סורוקה, שם הוא משמש כיום כסגן מנהל מכון הגסטרו.</p>
      ...
      <section className="research section" id="research">
        <div className="sectionHead splitHead researchHead">
          ...
          <p>ד״ר סגל פעיל במחקרים קליניים בתחומי מחלות המעי הדלקתיות והמיקרוביום.</p>
        </div>

        <div className="researchGrid">
          {researchItems.map((item, index) => (
            <a className="researchCard" href={item.href} target="_blank" rel="noreferrer" key={item.title}>
              <div className="researchCardTop"><span className="researchSource">{item.source}</span><span className="researchIndex">0{index + 1}</span></div>
              <h3>{item.title}</h3>
              <p>{item.text}</p>
      ...
      <section className="insurance section" id="insurance">
        <div className="sectionHead insuranceHead">
          <div>
        ...
        <div className="insuranceLogos" aria-label="הסדרים באסותא">
          {insuranceArrangements.map((item) => (
            <div className={`insuranceLogo ${item.className}`} key={item.name}><span>{item.name}</span></div>
          ))}
        </div>
      </section>
      ...
      <section className="reviewSection" id="reviews">
        <div className="reviewInner">
          ...
          <a className="reviewLink" href="https://www.medreviews.co.il/provider/dr-segal-arik" target="_blank" rel="noreferrer">לצפייה בחוות הדעת ב־MedReviews ←</a>
        </div>
      </section>
      ...
      <section className="clinic section" id="clinic">
        <div className="clinicCopy">
          ...
          <p>זימון תורים מתבצע דרך אסותא. לשאלות כלליות ניתן לפנות למרפאה בטלפון או ב־WhatsApp.</p>
          <div className="actions">
      ...
      <footer>
        ...
        <div className="footerRight"><span>יצחק בן צבי 9, באר שבע</span><span>WhatsApp: {whatsappDisplay}</span><span>טלפון: {phoneDisplay}</span></div>
      </footer>
    </main>
  );
}
